...
export function addReadinessDimensions({
  ...
  addSchemasLadderDimension({ push, mustExist, heavy, fx, root });
  addMultiImplementationDimension({ push, mustExist, heavy, fx });
  addExternalEcosystemDimension({ push, mustExist, heavy, fx });
  addWorkbenchDailyDimension({ push, mustExist, read, heavy, fx });
  addFederationDimension({ push, mustExist, readSourceFamily, heavy, fx });
...
function addSchemasLadderDimension({ push, mustExist, heavy, fx, root }) {
  const subset = countLabanSubsetFixtures(root);
  const { ladder, motif, labanSchema, index } = schemaStatus(mustExist);
  ...
  const gateEvidence = schemaGateEvidence({ heavy, fx });
  push({
...
function schemaGateEvidence({ heavy, fx }) {
  return heavy ? [fx.schemas?.ok ? fx.schemas.detail : "schemas-fail"] : [];
}
...
function addMultiImplementationDimension({ push, mustExist, heavy, fx }) {
...
function addExternalEcosystemDimension({ push, mustExist, heavy, fx }) {
  const files = [
...
function addWorkbenchDailyDimension({ push, mustExist, read, heavy, fx }) {
  const files = [
  ...
  const present = presentFiles(files, mustExist);
  const { sync, engraving } = workbenchSourceSignals({ files, mustExist, read });
  const { modes, print } = workbenchCapabilities({ sync, engraving });
  ...
  const gateEvidence = workbenchGateEvidence({ heavy, fx });
  push({
    id: "mveiWorkbench-daily",
    label: "MvEI Workbench local authoring",
    target: "strong",
    level,
    evidence: workbenchEvidence({ present, modes, print, gateEvidence }),
  });
}

function workbenchSourceSignals({ files, mustExist, read }) {
  const sync = mustExist(files[2]) ? read(files[2]) : "";
  const engraving = mustExist(files[4]) ? read(files[4]) : "";
  return { sync, engraving };
}

function workbenchCapabilities({ sync, engraving }) {
  return {
    modes:
      /broadcast/.test(sync) && /yjs/.test(sync) && /memory/.test(sync),
    print: /renderMotifPrintHtml|renderMotifToSvg/.test(engraving),
  };
}

function workbenchGateEvidence({ heavy, fx }) {
  return heavy
    ? [
...
function workbenchEvidence({ present, modes, print, gateEvidence }) {
  return [
...
function addFederationDimension({
  push,
  mustExist,
  readSourceFamily,
  heavy,
...
function addRepositoryGovernanceDimension({ push, mustExist, read, heavy, fx }) {
  ...
  const present = presentFiles(files, mustExist);
  const sources = governanceSources({ files, mustExist, read });
  const processOk = governanceProcessOk(sources);
  ...
  const gateEvidence = governanceGateEvidence({ heavy, fx });
  push({
    id: "repository-governance",
    label: "Repository governance status",
    target: "strong",
    level,
    evidence: governanceEvidence({ present, processOk, gateEvidence }),
    nonClaim: "Repository process only, with no named maintainers or external consortium",
  });
}

function governanceSources({ files, mustExist, read }) {
  ...
  return { maintainers, dualRfc, consortium };
}

function governanceProcessOk({ maintainers, dualRfc, consortium }) {
  return (
...
function governanceGateEvidence({ heavy, fx }) {
  return heavy
    ? [
...
function governanceEvidence({ present, processOk, gateEvidence }) {
  return [
    ...present.map((file) => `exists:${file}`),
    processOk
      ? "unassigned roles + dual review + external non-claim"
      : "process incomplete",
    ...gateEvidence,
  ];
}
